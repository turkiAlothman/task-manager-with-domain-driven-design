...
namespace TaskManager.Services.Implementers
{
    public class EmailService : IEmailService
    {
        ...
        public  EmailService(IOptions<MailSettings> settings, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task SendInviteEmail(Employees inviter, string inviteeEmail, string SecretKey)
        {
            await this.SendMail(new Mail { Body = @$"
                    ...
                    <div style=""max-width: 600px; margin: 0 auto; background-color: #fff; padding: 30px; border-radius: 10px; box-shadow: 0 0 10px rgba(0,0,0,0.1);"">
                        ...
                        <div style=""text-align: center; margin-top: 20px;"">
                            <a href="" https://{httpContextAccessor.HttpContext.Request.Host.Value}/auth/RegisterationRedirect/{inviteeEmail}/{SecretKey}""  style=""display: inline-block; padding: 10px 20px; background-color: #924AEF; color: #fff; text-decoration: none; border-radius: 5px;"">Register Now</a>
                        </div>
                    </div>
                            " , EmailTo = inviteeEmail , Subject= "Account Registration" });
        }

        public async Task SendResetPasswordEmail(string Email, string SecretKey)
        {
            await this.SendMail(new Mail { Body = @$"
                    ...
                    <div style=""max-width: 600px; margin: 0 auto; background-color: #fff; padding: 30px; border-radius: 10px; box-shadow: 0 0 10px rgba(0,0,0,0.1);"">
                        ...
                        <div style=""text-align: center; margin-top: 20px;"">
                            <a href="" https://{httpContextAccessor.HttpContext.Request.Host.Value}/auth/ResetPasswordRedirect/{Email}/{SecretKey}""  style=""display: inline-block; padding: 10px 20px; background-color: #924AEF; color: #fff; text-decoration: none; border-radius: 5px;"">Reset Password</a>
                        </div>
                    </div>
